...
class DecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.multi_head_attention_cognition_dec = MultiHeadAttention(
            hidden_size,
            total_key_depth,
            total_value_depth,
            hidden_size,
        ...
        self.multi_head_attention_affection_dec = MultiHeadAttention(
            hidden_size,
            total_key_depth,
            total_value_depth,
            hidden_size,
        ...
        self.layer_norm_mha_cognition = LayerNorm(hidden_size)
        self.layer_norm_mha_affection = LayerNorm(hidden_size)
        self.layer_norm_ffn = LayerNorm(hidden_size)
    ...
    def forward(self, inputs):
        ...
        x, encoder_outputs, attention_weight, mask, cs_enc_outputs, cs_enc_mask, concept_enc_outputs, concept_enc_mask = inputs
        ...
        # Layer Normalization before encoder-cognition cross attention
        x_norm = self.layer_norm_mha_enc(x)
        
        # cross attention: cognition
        y, _ = self.multi_head_attention_cognition_dec(
            queries=x_norm, 
            keys=cs_enc_outputs, 
            values=cs_enc_outputs, 
            mask=cs_enc_mask
        )
      
        # Dropout and residual after cross-attention
        x = self.dropout(x + y)
        
        # Layer Normalization before encoder-cognition-affection cross attention
        x_norm = self.layer_norm_mha_cognition(x)
        
        # cross attention: affection
        y, _ = self.multi_head_attention_affection_dec(
            queries=x_norm, 
            keys=concept_enc_outputs, 
            values=concept_enc_outputs, 
            mask=concept_enc_mask
        )
        ...
        x_norm = self.layer_norm_mha_affection(x)
    ...
    with torch.no_grad():
        ...
        for j, batch in pbar:
            if config.model == "case":
                ...
                raise ValueError(config.model + " Error!")
            # l.append(loss)
            # p.append(ppl)
            # bce.append(bce_prog)
            # acc.append(acc_prog)
            ...
            pbar.set_description(
                "loss:{:.4f} ppl:{:.1f}".format(np.mean(ctx_loss), math.exp(np.mean(ctx_loss)))
            )

    bow_loss = np.mean(bow_loss)
    kl_loss = np.mean(kl_loss)
    mim_loss = np.mean(mim_loss)
    ctx_loss = np.mean(ctx_loss)
    ppls = np.mean(ppls)
    str_loss = np.mean(str_loss)
